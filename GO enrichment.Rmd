...
if (tools::md5sum("basic_analysis.RData") == "b38dbde28a5928d494dd1b58bf737c01"){
...
ggsave(filename = "outputs/GO_upset.png", width = 8, height= 5)
```
...
plot_genes_from_GO <- function(GO_term_to_plot, title) {
  ...
  left_join(all_wt_sig_flat, by = c("locus_tag", "species")) |>
    mutate(
      ...
      significant = replace(significant, is.na(significant), FALSE)
    ) |>
...
plot_gene_from_custom_set <- function(locus_family_of_interest, title = " ") {
    all_wt_sig_flat |>
      ...
      geom_hline(yintercept = 0) +
      ggtitle(title) +
      labs(y = "log2 fold change (15°C/37°C)") +
    theme_classic() +
      theme(axis.text.x = element_text(face = "italic")) +
      scale_color_brewer(palette = "Dark2")
  }
...
```{r}
go_0009239 <- plot_genes_from_GO(GO_term_to_plot = "GO:0009239", title = "enterobactin biosynthetic process")
...
ggsave(filename = "outputs/iron_combined.png", height = 10, width = 10)
```
...
```{r}
tca_plot + glycine_plot + osmo_plot + biofilm_plot +  plot_layout(guides = 'collect',
              axes = "collect",
              axis_titles = "collect") +
  plot_annotation(tag_levels = 'A') &
  theme(plot.title = element_text(size = 10),
...
ggsave(filename = "outputs/misc_GO.png", height = (20/3), width = 10)

```
This is a heterogenous bunch, including otsBA (trehalose biosynthesis), lpxABCP (lipid A biosynthesis - lipid A is what anchors LPS to the membrane)
...
ggsave(filename = "outputs/ribosome_proton_transport.jpg", width = 7, height = 4)
...
colanic_acid_custom_plot <- colanic_acid_custom_set |>
  plot_gene_from_custom_set(title = "colanic acid associated genes") + theme(axis.text.x = element_text(angle = 0))
...
ggsave(filename = "outputs/colanic_acid.png", height = 7, width = 7.5)
...
Below I explore for Serratia some of the unique GO terms. Plots make it look like none of these are particularly unique. 


```{r}
enriched_GO_overview |> 
  filter(number_of_genomes == 1, S_marcescens_Db11 == 1)
```
```{r}
plot_genes_from_GO(GO_term_to_plot = "GO:0006096", title = "")
```
...
```{r}
plot_gene(ecoli_gene_name = "csrA")
```

https://ecocyc.org/gene?orgid=ECOLI&id=EG12092
https://ecocyc.org/gene?orgid=ECOLI&id=EG10181




Trehalose metabolism seems to be more conserved.

```{r}
plot_genes_from_GO("GO:0005991", "trehelose metabolic process")
```

Finally, this is a big term, but I notice that E. coli has more upregualted genes involved in amino acid metabolism:

```{r}
plot_genes_from_GO(GO_term_to_plot = "GO:1901607", title = "alpha-amino acid biosynthetic process")
```
These may have already been captured in the histadine biosynthesis plus some others.


nucleic acid transport

```{r}
#plot_genes_from_GO(GO_term_to_plot = "GO:0050657", title = "nucleic acid transport")
plot_gene(ecoli_gene_name = "sxy")
```
