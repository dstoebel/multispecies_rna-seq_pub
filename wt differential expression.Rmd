...
```
### What are the locus_tag for each locus_family

```{r}
locus_tags_per_family <- all_wt_sig_flat |>
  dplyr::select(locus_family, species, locus_tag) |> 
  mutate(species = format_species_names(species)) |> 
  pivot_wider(names_from = species, values_from = locus_tag) |> 
  rowwise() |> 
  mutate(num_genomes = sum(!is.na(c_across(2:7)))) |> 
  ungroup()
  
 locus_tags_per_family |>  
  arrange(desc(num_genomes)) |> 
  write_tsv("outputs/locus_tag_per_locus_family.tsv")

```

### What are the core locus families? 
...
### Calculate the number of genes in each grouping, and also calculate the percentages
```{r}

locus_fam_num_genomes <- all_wt_sig_flat |> 
  dplyr::count(locus_family) |> 
  dplyr::rename(num_genomes_with_locus_fam = n)
...
all_up_locus_fams <-  all_wt_sig_flat |> 
  ...
  filter(num_up == 6) |> 
  left_join(filter(all_wt_sig_flat, species == "E_coli_K12"))|> 
   dplyr::select(locus_family, `E. coli locus tag` = locus_tag, gene) |> 
  mutate(direction_of_regulation = "up")
...
all_down_locus_fams <- all_wt_sig_flat |> 
  ...
  left_join(filter(all_wt_sig_flat, species == "E_coli_K12"))|> 
   dplyr::select(locus_family, `E. coli locus tag` = locus_tag, gene) |> 
  mutate(direction_of_regulation = "down")


bind_rows(all_up_locus_fams,all_down_locus_fams) |> 
  write_tsv("outputs/genes_DE_in_6_species.tsv")
```
